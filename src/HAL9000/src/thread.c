...
)
{
	...
	*ResultingStack = (PVOID)PtrDiff(InitialStack, SHADOW_STACK_SIZE + sizeof(PVOID));
	...
	char* str = Process->FullCommandLine;

	QWORD argc = (QWORD)Process->NumberOfArguments;

	QWORD StackAllignment = (QWORD)0;
	...
	char* argv[256];
	PVOID* adr = (PVOID*)ExAllocatePoolWithTag(0, Process->NumberOfArguments * sizeof(PVOID), HEAP_TEST_TAG, 0);

	DWORD i = 0;

	QWORD Size = (QWORD)0;

	token = strtok_s(str, delimiters, &rest);

	while (token != NULL) {

		argv[i] = (char*)token;

		*ResultingStack = (PVOID)PtrDiff(*ResultingStack, (QWORD)strlen(token) + (QWORD)1);

		Size += (QWORD)strlen(token) + (QWORD)1;
		i++;
		token = strtok_s(NULL, delimiters, &rest);
	}

	if (argc % 2 == 0) {

		QWORD aux = (QWORD)*ResultingStack;
		QWORD LastDigit = aux % 16;
		*ResultingStack = (PVOID)PtrDiff(*ResultingStack, LastDigit);
		StackAllignment = LastDigit;
		Size += LastDigit;
	}
	...
	if (argc % 2 == 1) {
		...
		QWORD aux = (QWORD)*ResultingStack;
		QWORD LastDigit = aux % 16;
		if (LastDigit >= (QWORD)8) {
			QWORD diff = LastDigit - (QWORD)8;
			*ResultingStack = (PVOID)PtrDiff(*ResultingStack, diff);
			StackAllignment = diff;
			Size += diff;
		}
		else {
			*ResultingStack = (PVOID)PtrDiff(*ResultingStack, LastDigit + (QWORD)8);
			StackAllignment = LastDigit + (QWORD)8;
			Size += LastDigit + (QWORD)8;
		}

	}

	*ResultingStack = (PVOID)PtrDiff(*ResultingStack, argc * sizeof(PVOID));
	Size += (QWORD)argc * sizeof(PVOID);

	*ResultingStack = (PVOID)PtrDiff(*ResultingStack, 5 * sizeof(PVOID));
	Size += (QWORD)5 * sizeof(PVOID);


	MmuGetSystemVirtualAddressForUserBuffer(*ResultingStack, Size, PAGE_RIGHTS_READWRITE, Process, &KernelAddress);

	PVOID KernelAddressBackup = KernelAddress;

	INTR_STATE dummy;
	DWORD noOfFrames = (DWORD)(Size / PAGE_SIZE);
	LockAcquire(&Process->UsedFramesLock, &dummy);
	Process->UsedFramesCount += (QWORD)noOfFrames;
	LockRelease(&Process->UsedFramesLock, dummy);

	KernelAddress = (PVOID)PtrOffset(KernelAddress, Size);

	PVOID KernelEndOfMapping = KernelAddress;

	QWORD j = argc;


	//first push the arguments on the stack, the one at the highest address being the last arg

	while (j >= (QWORD)1) {

		j--;
		KernelAddress = (PVOID)PtrDiff(KernelAddress, (QWORD)strlen(argv[j]) + (QWORD)1);
		QWORD DifferenceFromEnd = (QWORD)PtrDiff(KernelEndOfMapping, KernelAddress);
		adr[j] = (PVOID)PtrDiff(InitialStack, DifferenceFromEnd);
		memcpy(KernelAddress, argv[j], (QWORD)strlen(argv[j]) + (QWORD)1);
	}


	KernelAddress = (PVOID)PtrDiff(KernelAddress, StackAllignment);

	j = argc;

	//then push the addresses of the arguments pushed before

	while (j >= (QWORD)1) {

		j--;
		KernelAddress = (PVOID)PtrDiff(KernelAddress, sizeof(PVOID));
		memcpy(KernelAddress, &adr[j], sizeof(adr[j]));

	}

	QWORD DiffernceFromEnd = (QWORD)PtrDiff(KernelEndOfMapping, KernelAddress);
	PVOID Aux = (PVOID)PtrDiff(InitialStack, DiffernceFromEnd);


	KernelAddress = (PVOID)PtrDiff(KernelAddress, 3 * sizeof(PVOID));



	//push the address of where the address of the first argument was pushed on the stack
	memcpy(KernelAddress, &Aux, sizeof(PVOID));
	...
	memcpy(KernelAddress, &argc, sizeof(PVOID));

	KernelAddress = (PVOID)PtrDiff(KernelAddress, sizeof(PVOID));


	MmuFreeSystemVirtualAddressForUserBuffer(KernelAddressBackup);

	ExFreePoolWithTag(adr, HEAP_TEST_TAG);
